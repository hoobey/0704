...
<TABLE id=table2 cellSpacing=0 cellPadding=0 width="100%" border=0>
  ...
  <TR>
    ...
    <%
       System.out.println("========================");
    List commimalist=(List)request.getAttribute("commimalist");
    System.out.println("几个"+commimalist.size());
	Map yhMap=(Map)commimalist.get(0);
				...
				if(rmm.value=="")
				{
					...
					if(sf==false)
					{
						alert("输入错误，请按提示输入");
							...
							document.thisForm.action="<%=request.getContextPath()%>/comgaimima?mima="+pass;
	...
	function ducuo(lx,d,obj)
	{
	 var div1=hq(d);
	 		if(lx=="jiumima")
			{
						...
						if(obj.value!='<%=((String)yhMap.get("com_login_pass"))%>')
						{
							yhmc1.src="<%=request.getContextPath()%>/images/cha.gif";
